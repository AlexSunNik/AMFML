...
from modulate_bn import *

def get_afrm_param_reg_loss(model):
    total = 0
    for m in model.modules():
        if isinstance(m, afrm_BN):
            weight_loss, bias_loss = m.return_loss()
            total += weight_loss + bias_loss
    return total
################################################################################################
# Regular Multitask Training Loss
class SceneNetLoss(nn.Module):
    def __init__(self, dataset, tasks, tasks_num_class, lambdas, device, data_root=None, afrm_reg=False, afrm_reg_coef=0.0001):
        super().__init__()
        self.device = device
        self.tasks = tasks
        ...
        self.afrm_reg = afrm_reg
        self.afrm_regcoef = afrm_reg_coef
        print("Afrm Reg Coef:", self.afrm_regcoef)
    ################################################################################################
    def forward(self, preds, targets, model=None, is_train=False):
        total_loss = 0
        ...
        if self.afrm_reg and is_train:
            assert model is not None
            reg_loss = get_afrm_param_reg_loss(model)
            print("AFRM Reg Loss:", round(float(reg_loss.item()), 3))
            total_loss += self.afrm_regcoef * reg_loss
        return total_loss
    ###########################################################################################
