...
def replace_bn(m, name):
    for attr_str in dir(m):
        ...
        if type(target_attr) == torch.nn.BatchNorm2d:
            ...
            setattr(m, attr_str, afrm_BN(target_attr.num_features))
...
class SceneNet(nn.Module):
    ...
    def forward(self, img, task_id, num_train_layers=None, hard_sampling=False, mode='train'):
        for m in self.modules():
            if isinstance(m, afrm_BN):
                # print("Change Task Id.")
