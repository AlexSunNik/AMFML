...
import matplotlib.pyplot as plt
from scene_net_modulated import *
...
parser.add_argument('--dest', default='/data/alexsun/ckp/nyuv2/', type=str, help='Destination Save Folder.')
parser.add_argument('--afrm-reg', action='store_true')
parser.add_argument('--afrm-regcoef', type=float, help='AFRM Regularization CoEfficient', default="0.0001")
...
criterion = SceneNetLoss(dataset, TASKS, TASKS_NUM_CLASS, LAMBDAS, device, DATA_ROOT, afrm_reg=afrm_reg, afrm_reg_coef=args.afrm_regcoef)
...
batch_update = 16
net = train(net, dataset, criterion, optimizer, scheduler, train_loader, test_loader, network_name, batch_update, max_iters=MAX_ITERS, log_file=log_file, save_model=True, method="baseline", dest=dest, afrm=True)
...
net.load_state_dict(torch.load(f"{dest}/final_{network_name}.pth"))
net.eval()
res = evaluator.get_final_metrics(net, test_loader, afrm=True)
